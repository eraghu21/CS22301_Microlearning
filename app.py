...
# Optional: paste full base64 of your image here, leave empty if no background
CERT_BG_BASE64 = ""  
...
def create_certificate(name, reg_no, subject):
    ...
    # ----------------- Background from base64 (optional) -----------------
    if CERT_BG_BASE64.strip():  # only use if base64 is non-empty
        try:
            bg_bytes = base64.b64decode(CERT_BG_BASE64.strip())
            bg_image = Image.open(io.BytesIO(bg_bytes)).convert("RGB")
            bg_path = f"{reg_no}_bg_temp.png"
...
if st.session_state.get("timer_started", False):
    ...
    # "I have watched the video" button
